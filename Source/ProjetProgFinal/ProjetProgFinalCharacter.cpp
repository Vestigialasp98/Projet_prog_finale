...
AProjetProgFinalCharacter::AProjetProgFinalCharacter()
{
	...
	// --- MODIFICATION POUR CAMÃ‰RA STATIQUE ---

	// Create a camera boom (pulls in towards the player if there is a collision)
	CameraBoom = CreateDefaultSubobject<USpringArmComponent>(TEXT("CameraBoom"));
	CameraBoom->SetupAttachment(RootComponent);
	
	// Longueur de la "perche" de la camÃ©ra (distance par rapport au joueur)
	CameraBoom->TargetArmLength = 1000.0f; 
	
	// On fixe la rotation du boom pour qu'il regarde d'en haut (ex: -70 degrÃ©s)
	CameraBoom->SetRelativeRotation(FRotator(-70.0f, 0.0f, 0.0f));

	// On DÃ‰SACTIVE la rotation du boom par la souris/contrÃ´leur
...
void AProjetProgFinalCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	if (UEnhancedInputComponent* EnhancedInputComponent = Cast<UEnhancedInputComponent>(PlayerInputComponent)) {
		...
		// --- MODIFICATION : On dÃ©sactive l'input de la souris pour la camÃ©ra ---
		// Looking
...
{
	// (Cette fonction n'est plus appelÃ©e, mais on la laisse au cas oÃ¹)
